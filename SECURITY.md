...
- Use the Silc security email ([security@evmos.org](mailto:security@evmos.org)) to disclose all vulnerabilities,
...
Silc uses the following disclosure process:
...
2. The team determines the vulnerability’s potential impact on Silc.
...
- For `High` and `Critical`:
    1. Patches are prepared for supported releases of Silc in a
    ...
    4. The nodes and validators update their Silc and Ethermint dependencies to use these releases.
    5. A week (or less) after the security vulnerability has been patched on Silc,
...
including but not limited to Silc,
as secure as possible.

### Payment Process

The payment process will be executed according to Silc SAFU for `Critical` and `High` severity vulnerabilities.
Payouts can only be executed in accordance and under supervision of the Silc Operations team and only once the
...
KYC/KYB result
(Accepted or Rejected) to the Silc Core Team. The Silc Core team does not have access to any of the information
...
The team commits to releasing security patch releases for the latest release that Silc is running.
...
are welcomed to backport security patches to older versions for their own use, the Silc team reserves
...
The Silc Security Team is constantly being monitored.
