...
import (
	...
	"github.com/silcprotocol/silc/testutil/tx"
)
...
//  3. EndBlock
//  4. Commit
func Commit(ctx sdk.Context, app *app.Silc, t time.Duration, vs *cmttypes.ValidatorSet) (sdk.Context, error) {
...
func CommitAndCreateNewCtx(ctx sdk.Context, app *app.Silc, t time.Duration, vs *cmttypes.ValidatorSet) (sdk.Context, error) {
	header, err := commit(ctx, app, t, vs)
	if err != nil {
		return ctx, err
	}
...
func DeliverTx(
	...
	return BroadcastTxBytes(appSilc, txConfig.TxEncoder(), tx)
}
...
func DeliverEthTx(
	...
	tx, err := tx.PrepareEthTx(txConfig, appSilc, priv, msgs...)
	if err != nil {
		return abci.ExecTxResult{}, err
	}
	res, err := BroadcastTxBytes(appSilc, txConfig.TxEncoder(), tx)
	if err != nil {
		return res, err
	}

	codec := appSilc.AppCodec()
...
func CheckTx(
	...
	return checkTxBytes(appSilc, txConfig.TxEncoder(), tx)
}
...
func BroadcastTxBytes(app *app.Silc, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ExecTxResult, error) {
...
func commit(ctx sdk.Context, app *app.Silc, t time.Duration, vs *cmttypes.ValidatorSet) (tmproto.Header, error) {
...
func checkTxBytes(app *app.Silc, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseCheckTx, error) {
	bz, err := txEncoder(tx)
	if err != nil {
		return abci.ResponseCheckTx{}, err
	}
