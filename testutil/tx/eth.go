...
func PrepareEthTx(
	...
	signer := ethtypes.LatestSignerForChainID(appSilc.EvmKeeper.ChainID())
	txFee := sdk.Coins{}
	txGasLimit := uint64(0)
...
func CreateEthTx(
	...
	chainID := appSilc.EvmKeeper.ChainID()

	// When we send multiple Ethereum Tx's in one Cosmos Tx, we need to increment the nonce for each one.
	nonce := appSilc.EvmKeeper.GetNonce(ctx, fromAddr) + uint64(nonceIncrement) //nolint:gosec // G115
	evmTxParams := &evmtypes.EvmTxArgs{
		...
		Amount:    amount,
		GasLimit:  100000,
		GasFeeCap: appSilc.FeeMarketKeeper.GetBaseFee(ctx),
