...
- [Evmos Contributor Guidelines](#silc-contributor-guidelines)
...
1. Either [open](https://github.com/silc/silc/issues/new/choose)
   or [find](https://github.com/silc/silc/issues) an issue you have identified and would like to contribute to
   resolving.
...
3. If you would like to contribute:
    ...
    4. Follow standard Github best practices:
        1. Fork the repo
        2. Branch from the HEAD of `development`(For core developers working within the silc repo, to ensure a
...
Check out our [good first issues](https://github.com/silc/silc/issues?q=label%3A%22good+first+issue%22).
...
While my fork lives at `https://github.com/silc/silc`,
the code should never exist at `$GOPATH/src/github.com/silc/silc`.
Instead, we use `git remote` to add the fork as a new remote for the original repo,`$GOPATH/src/github.com/silc/silc`,
...
2. Go to the original repo checked out locally. (i.e. `$GOPATH/src/github.com/silc/silc`)
3. `git remote rename origin upstream`
4. `git remote add origin git@github.com:tharsis/silc.git`

Now `origin` refers to my fork and `upstream` refers to the silc version.
So I can `git push -u origin master` to update my fork,
and make pull requests to silc from there.
...
4. Create your feature branch from `development` either on `github.com/silc/silc`, or your fork (
