...
// AppName defines the Ledger app used for signing. Silc uses the Ethereum app
const AppName = "Ethereum"

var (
	// SupportedAlgorithms defines the list of signing algorithms used on Silc:
	//  - eth_secp256k1 (Ethereum)
	SupportedAlgorithms = keyring.SigningAlgoList{hd.EthSecp256k1}
	// SupportedAlgorithmsLedger defines the list of signing algorithms used on Silc for the Ledger device:
	...
	// LedgerDerivation defines the Silc Ledger Go derivation (Ethereum app with EIP-712 signing)
	LedgerDerivation = ledger.SilcLedgerDerivation()
