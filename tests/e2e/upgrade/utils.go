...
// SilcVersions is a custom comparator for sorting semver version strings.
type SilcVersions []string

// Len is the number of stored versions.
func (v SilcVersions) Len() int { return len(v) }

// Swap swaps the elements with indexes i and j. It is needed to sort the slice.
func (v SilcVersions) Swap(i, j int) { v[i], v[j] = v[j], v[i] }

// Less compares semver versions strings properly
func (v SilcVersions) Less(i, j int) bool {
...
func CheckUpgradeProposalVersion(version string) ProposalVersion {
	...
	cmp := SilcVersions([]string{version, "v10.0.0", "v20.0.0"})
...
func RetrieveUpgradesList(upgradesPath string) ([]string, error) {
	...
	sort.Sort(SilcVersions(versions))

	return versions, nil
}

// ExportState executes the  'docker cp' command to copy container .silcd dir
...
func (m *Manager) ExportState(targetDir string) error {
	...
	cmd := exec.Command(
		"docker",
		"cp",
		fmt.Sprintf("%s:/root/.silcd", m.ContainerID()),
		targetDir,
	)
	return cmd.Run()
}
