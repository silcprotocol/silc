...
// WaitForHeight queries the Silc node every second until the node will reach the specified height.
...
// GetNodeHeight calls the Silc CLI in the current node container to get the current block height
func (m *Manager) GetNodeHeight(ctx context.Context) (int, error) {
	cmd := []string{"silcd", "q", "block"}
	...
	if !SilcVersions([]string{m.CurrentVersion, "v20.0.0"}).Less(0, 1) {
...
// UnwrapBlockHeightPreV50 unwraps the block height from the output of the silcd query block command
...
// UnwrapBlockHeightPostV50 unwraps the block height from the output of the silcd query block command
...
// GetNodeVersion calls the Silc CLI in the current node container to get the
// current node version
func (m *Manager) GetNodeVersion(ctx context.Context) (string, error) {
	exec, err := m.CreateExec([]string{"silcd", "version"}, m.ContainerID())
	if err != nil {
...
func (m *Manager) getTimeoutCommit(ctx context.Context) (*big.Int, error) {
	exec, err := m.CreateExec([]string{"grep", `\s*timeout_commit =`, "/root/.silcd/config/config.toml"}, m.ContainerID())
	if err != nil {
