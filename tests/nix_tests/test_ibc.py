...
def test_ibc_transfer_with_hermes(ibc):
    ...
    old_dst_balance = get_balance(ibc.chains["silc"], dst_addr, dst_denom)
    new_dst_balance = 0

    def check_balance_change():
        nonlocal new_dst_balance
        new_dst_balance = get_balance(ibc.chains["silc"], dst_addr, dst_denom)
    ...
    fee = int(events["tx"]["fee"].removesuffix("sillet"))
...
def test_silc_ibc_transfer(ibc):
    """
    test sending sillet from silc to crypto-org-chain using cli.
    """
    assert_ready(ibc)
...
def test_silc_ibc_transfer_acknowledgement_error(ibc):
    """
    test sending sillet from silc to crypto-org-chain using cli
