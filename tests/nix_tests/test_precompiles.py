...
from .ibc_utils import SILC_IBC_DENOM, assert_ready, get_balance, prepare_network
...
def test_ibc_transfer(ibc):
    ...
    old_src_balance = get_balance(ibc.chains["silc"], src_addr, src_denom)
    old_dst_balance = get_balance(ibc.chains["chainmain"], dst_addr, SILC_IBC_DENOM)
    ...
    gas_estimation = ibc.chains["silc"].w3.eth.estimate_gas(tx)
    receipt = send_transaction(ibc.chains["silc"].w3, tx, KEYS["signer2"])
    ...
    fee = receipt.gasUsed * silc_gas_price
...
def test_ibc_transfer_invalid_packet(ibc):
    ...
    try:
        tx = pc.functions.transfer(
            ...
            0,
            "",
        ).build_transaction({"from": ADDRS["signer2"], "gasPrice": silc_gas_price})
        send_transaction(ibc.chains["silc"].w3, tx, KEYS["signer2"])
...
def test_staking(ibc):
    ...
    old_src_balance = get_balance(silc, del_addr, src_denom)
    ...
    tx = pc.functions.delegate(ADDRS["signer2"], validator_addr, amt).build_transaction(
        {"from": ADDRS["signer2"], "gasPrice": silc_gas_price}
    )
    gas_estimation = silc.w3.eth.estimate_gas(tx)
    ...
    new_src_balance = get_balance(silc, del_addr, src_denom)
    assert old_src_balance - amt - fee == new_src_balance
...
def test_staking_via_sc(ibc):
    ...
    approve_tx = contract.functions.testApprove(
        ...
        {"from": ADDRS["signer1"], "gasPrice": silc_gas_price, "gas": 60000}
    )

    gas_estimation = silc.w3.eth.estimate_gas(approve_tx)
    ...
    delegate_tx = contract.functions.testDelegate(
        ...
        {"from": ADDRS["signer1"], "gasPrice": silc_gas_price, "gas": 180000}
    )
    gas_estimation = silc.w3.eth.estimate_gas(delegate_tx)
