...
def test_query_allowance(ibc, name, auth_args, args, err_contains, exp_res):
    ...
    gas_limit = 200_000
    pc = get_precompile_contract(ibc.chains["silc"].w3, "ICS20I")
    evmos_gas_price = ibc.chains["silc"].w3.eth.gas_price
...
def test_ibc_transfer_with_authorization(
    ...
    src_amount_evmos_prev = get_balance(ibc.chains["silc"], src_address, src_denom)
    ...
    src_amount_evmos_final = get_balance(ibc.chains["silc"], src_address, src_denom)
...
def test_ibc_transfer_from_eoa_through_contract(ibc):
    ...
    src_starting_balance = get_balance(ibc.chains["silc"], src_adr, "aevmos")
    ...
    src_final_amount_evmos = get_balance(ibc.chains["silc"], src_adr, src_denom)
...
def test_ibc_transfer_from_eoa_with_internal_transfer(
    ...
    src_starting_balance = get_balance(ibc.chains["silc"], src_bech32, "aevmos")
    dest_starting_balance = get_balance(
        ibc.chains["chainmain"], dst_addr, EVMOS_IBC_DENOM
    )
    ...
    src_final_amount_evmos = get_balance(ibc.chains["silc"], src_bech32, src_denom)
...
def test_ibc_multi_transfer_from_eoa_with_internal_transfer(
    ...
    # get starting balances to check after the tx
    src_starting_balance = get_balance(silc, src_bech32, "aevmos")
    dest_starting_balance = get_balance(
        ibc.chains["chainmain"], dst_addr, EVMOS_IBC_DENOM
    )
...
def setup_interchain_sender_contract(
    ...
    src_denom = "aevmos"
    gas_limit = 200_000
    evmos_gas_price = silc.w3.eth.gas_price
    # Deployment of contracts and initial checks
