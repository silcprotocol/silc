...
def ibc(request, tmp_path_factory):
    ...
    network = prepare_network(path, name, [silc_build, "chainmain"])
    yield from network
...
def test_denom_trace(ibc, name, args, err_contains, exp_res):
    ...
    # setup: send some funds from chain-main to silc
    ...
    try:
        # make the query call
        pc = get_precompile_contract(ibc.chains["silc"].w3, "ICS20I")
...
def test_query_allowance(ibc, name, auth_args, args, err_contains, exp_res):
    ...
    gas_limit = 200_000
    pc = get_precompile_contract(ibc.chains["silc"].w3, "ICS20I")
    silc_gas_price = ibc.chains["silc"].w3.eth.gas_price
    ...
    tx_receipt = send_transaction(ibc.chains["silc"].w3, approve_tx, KEYS["validator"])
...
def test_approve(ibc, name, args, exp_err, err_contains, exp_spend_limit):
    ...
    tx_receipt = send_transaction(ibc.chains["silc"].w3, approve_tx, KEYS["signer1"])
    if exp_err:
        assert tx_receipt.status == 0, f"Failed: {name}"
        # get the corresponding error message from the trace
        trace = debug_trace_tx(ibc.chains["silc"], tx_receipt.transactionHash.hex())
    ...
    cli = ibc.chains["silc"].cosmos_cli()
...
def test_revoke(ibc, name, args, exp_err, err_contains):
    ...
    try:
        ...
        tx_receipt = send_transaction(
            ibc.chains["silc"].w3, revoke_tx, KEYS["signer1"]
        )
    except Exception as err:
        check_error(err, err_contains)
        return
...
def test_increase_allowance(
    ...
    try:
        ...
        tx_receipt = send_transaction(
            ibc.chains["silc"].w3, incr_allowance_tx, KEYS["validator"]
        )
    except Exception as err:
        check_error(err, err_contains)
        return
...
def test_decrease_allowance(
    ...
    tx_receipt = send_transaction(ibc.chains["silc"].w3, approve_tx, KEYS["community"])
    assert tx_receipt.status == 1
    ...
    try:
        ...
        tx_receipt = send_transaction(
            ibc.chains["silc"].w3, decr_allowance_tx, KEYS["community"]
        )
    except Exception as err:
        check_error(err, err_contains)
        return
...
def test_ibc_transfer_with_authorization(
    ...
    if auth_coins is not None:
        ...
        receipt = send_transaction(ibc.chains["silc"].w3, approve_tx, KEYS["signer2"])
    ...
    src_amount_silc_prev = get_balance(ibc.chains["silc"], src_address, src_denom)
    dst_balance_prev = get_balance(
        ibc.chains["chainmain"], dst_address, SILC_IBC_DENOM
    )
    try:
        ...
        receipt = send_transaction(ibc.chains["silc"].w3, transfer_tx, KEYS["signer2"])
    except Exception as err:
        check_error(err, err_contains)
        return
    ...
    if exp_err:
        ...
        # get the corresponding error message from the trace
        trace = debug_trace_tx(ibc.chains["silc"], receipt.transactionHash.hex())
    ...
    assert receipt.status == 1, debug_trace_tx(
        ibc.chains["silc"], receipt.transactionHash.hex()
    )
    fees = receipt.gasUsed * silc_gas_price
    ...
    src_amount_silc_final = get_balance(ibc.chains["silc"], src_address, src_denom)

    assert src_amount_silc_final == src_amount_silc_prev - fees - transfer_amt
...
def test_ibc_transfer_from_eoa_through_contract(ibc):
    ...
    src_adr = ibc.chains["silc"].cosmos_cli().address("signer2")
    ...
    tx_receipt = send_transaction(ibc.chains["silc"].w3, approve_tx, KEYS["signer2"])
    assert tx_receipt.status == 1
    ...
    src_starting_balance = get_balance(ibc.chains["silc"], src_adr, "sillet")
    ...
    send_tx = eth_contract.functions.transferFromEOA(
        ...
        {"from": ADDRS["signer2"], "gasPrice": silc_gas_price, "gas": gas_limit}
    )
    receipt = send_transaction(ibc.chains["silc"].w3, send_tx, KEYS["signer2"])
    ...
    src_final_amount_silc = get_balance(ibc.chains["silc"], src_adr, src_denom)
    assert src_final_amount_silc == src_starting_balance - amt - fees
...
def test_ibc_transfer_from_eoa_with_internal_transfer(
    ...
    src_denom = "sillet"
    gas_limit = 200_000
    silc_gas_price = w3.eth.gas_price

    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    # address that will escrow the sillet tokens when transferring via IBC
    escrow_bech32 = silc.cosmos_cli().escrow_address(channel)
    ...
    eth_contract = setup_interchain_sender_contract(
        silc, ACCOUNTS["signer2"], amt, initial_contract_balance
    )

    # get starting balances to check after the tx
    src_bech32 = silc.cosmos_cli().address("signer2")
    contract_bech32 = eth_to_bech32(eth_contract.address)
    other_addr_bech32 = eth_to_bech32(other_addr)

    src_starting_balance = get_balance(ibc.chains["silc"], src_bech32, "sillet")
    dest_starting_balance = get_balance(
        ibc.chains["chainmain"], dst_addr, SILC_IBC_DENOM
    )
    other_addr_initial_balance = get_balance(
        ibc.chains["silc"], other_addr_bech32, src_denom
    )
    escrow_initial_balance = get_balance(ibc.chains["silc"], escrow_bech32, src_denom)
    ...
    if other_addr == src_addr:
        ...
        other_addr_final_balance = get_balance(
            ibc.chains["silc"], escrow_bech32, src_denom
        )
    ...
    src_final_amount_silc = get_balance(ibc.chains["silc"], src_bech32, src_denom)
    assert src_final_amount_silc == exp_src_final_bal, f"Failed {name}"

    # Check contracts final state (balance & counter)
    contract_final_balance = get_balance(
        ibc.chains["silc"], contract_bech32, src_denom
    )
    assert (
    ...
    escrow_final_balance = get_balance(ibc.chains["silc"], escrow_bech32, src_denom)
...
def test_ibc_multi_transfer_from_eoa_with_internal_transfer(
    ...
    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    escrow_bech32 = silc.cosmos_cli().escrow_address("channel-0")

    # Deployment of contracts and initial checks
    initial_contract_balance = 100
    eth_contract = setup_interchain_sender_contract(
        silc, ACCOUNTS["signer2"], ibc_transfer_amt, initial_contract_balance
    )
    ...
    # get starting balances to check after the tx
    src_starting_balance = get_balance(silc, src_bech32, "sillet")
    dest_starting_balance = get_balance(
        ibc.chains["chainmain"], dst_addr, SILC_IBC_DENOM
    )
    escrow_initial_balance = get_balance(silc, escrow_bech32, src_denom)
...
def test_multi_ibc_transfers_with_revert(ibc):
    ...
    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    escrow_bech32 = ibc.chains["silc"].cosmos_cli().escrow_address("channel-0")

    # Deployment of contracts and initial checks
    initial_contract_balance = 100
    interchain_sender_contract = setup_interchain_sender_contract(
        silc, ACCOUNTS["signer2"], ibc_transfer_amt * 2, initial_contract_balance
    )
    ...
    assert src_final_amount_silc == exp_src_final_bal

    # Check contracts final state (balance & counter)
    contract_final_balance = get_balance(
        ibc.chains["silc"], interchain_sender_contract_bech32, src_denom
    )
    assert (
        contract_final_balance
        == initial_contract_balance - exp_amt_tranferred_internally
...
def setup_interchain_sender_contract(
    silc, src_acc, transfer_amt, initial_contract_balance
):
    """
    ...
    src_denom = "sillet"
    gas_limit = 200_000
    silc_gas_price = silc.w3.eth.gas_price
    # Deployment of contracts and initial checks
    eth_contract, tx_receipt = deploy_contract(silc.w3, CONTRACTS["InterchainSender"])
    assert tx_receipt.status == 1

    counter = eth_contract.functions.counter().call()
    assert counter == 0

    pc = get_precompile_contract(silc.w3, "ICS20I")
    # Approve the contract to spend the src_denom
    approve_tx = pc.functions.approve(
    ...
    tx_receipt = send_transaction(silc.w3, approve_tx, src_acc.key)
    assert tx_receipt.status == 1
    ...
    contract_bech32 = eth_to_bech32(eth_contract.address)
    fund_tx = silc.cosmos_cli().transfer(
        src_bech32,
        contract_bech32,
        f"{initial_contract_balance}sillet",
        gas_prices=f"{silc_gas_price + 100000}sillet",
        generate_only=True,
    )

    fund_tx = silc.cosmos_cli().sign_tx_json(fund_tx, src_bech32, max_priority_price=0)
    rsp = silc.cosmos_cli().broadcast_tx_json(fund_tx, broadcast_mode="sync")
    assert rsp["code"] == 0, rsp["raw_log"]
    txhash = rsp["txhash"]
    wait_for_new_blocks(silc.cosmos_cli(), 2)
    receipt = silc.cosmos_cli().tx_search_rpc(f"tx.hash='{txhash}'")[0]
